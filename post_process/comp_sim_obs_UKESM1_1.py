...
def new_name(name:str) -> str:
    ...
    if _name_pat is None:
        _name_pat = re.compile(r'^(.*?)_(\d+)$')  # should match _digits at the end with the digits as one of the matches
...
def change_name(var:typing.Union[xarray.DataArray,xarray.Variable],clear:bool=False) -> str:
    ...
    :param var: Xarray Varibale or DataArray to see if already have
...
def read_UMfiles(files:typing.Iterable[pathlib.Path]) -> xarray.Dataset:
    ...
    for cube in cubes:
        ...
        try:
            ...
            pass
        # potentially expand some co-ords
...
def merge_dataArray(dataArray_list: typing.List[xarray.DataArray]) -> xarray.Dataset:
    ...
    for indx, da in enumerate(dataArray_list):  # loop over list
        ...
        cleaned.append(da.rename(rename_dict))  # rename
...
def guess_lat_lon_vert_names(dataArray):
    ...
    vert_patterns = ['atmosphere_hybrid_sigma_pressure_coordinate', 'altitude', 'pressure','air_pressure','model_level_number']
...
def total_column(data:typing.Optional[xarray.DataArray],
    ...
    col = (data * atmos_mass).sum(vertical_coord)/area  # work out mass by integrating.
...
def genProcess(dataset, land_mask):
    """
    ...
    coord_500hPa = dict(pressure=500e3)  # co-ord value for 500 hPa
    coord_50hPa = dict(pressure=50e3)  # co-ord value for 50# hPa
    # set up the data to be meaned. Because of xarray's use of dask no loading happens till
    # data actually processed (below)
    mass = dataset.get("m01s50i063")
    process = {
        'TEMP@50': name_fn('m01s30i204',dataset,coord_50hPa,name_type='stash'),
        'TEMP@500': name_fn('m01s30i204',dataset,coord_500hPa,name_type='stash'),
        'RH@500': name_fn('m01s30i206',dataset,coord_500hPa,name_type='stash'),
...
def compute_values(files:typing.Iterable[pathlib.Path],
                   ...
                   clean_files:typing.Iterable[pathlib.Path]=None,
                   verbose:int =0) -> typing.Dict:
    """
    ...
    :param clean_files:  list of files to remove.
    :param verbose: if >0 be verbose.
    :return: dict of results
    """
    dataset = read_UMfiles(files)
    dataset = dataset.sel(time=slice(start_time, end_time))
    land_mask = dataset.land_area_fraction.isel(time=0).squeeze()  # land/sea mask
    land_mask = xarray.where(land_mask > 0.5,True,False)
...
def do_work():
    ...
    uses the config file postProcess block with the following options:'share/data/History_Data/'

        dir -- Path to where data is relative to cwd.  Only used if cmd line arg dir not specified with default value  'share/data/History_Data/'
        ...
        output -- if OUPUT not provided on cmd line will define output file. Must be provided if OUTPUT not provided.
    ...
    results= compute_values(files,output_file,start_time=start_time,end_time=end_time,clean_files=clean_files,verbose=verbose)
